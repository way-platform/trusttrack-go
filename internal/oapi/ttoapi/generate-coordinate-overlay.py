...
def extract_enum_values(description):
    ...
    for word in words:
        word = word.strip('.,;()<>')
        if ((word.isupper() and len(word) > 1) or '_' in word) and word not in ['BR', 'AND', 'OR']:
...
def map_field_to_openapi(field):
    ...
    if field_type == 'number':
        ...
        return {'type': 'string', 'format': 'date-time', 'description': clean_description}
    elif field_type == 'array':
        return {'type': 'array', 'items': {'type': 'string'}, 'description': clean_description}
    elif field_type == 'enum':
        enum_values = extract_enum_values(description)
        if enum_values:
            return {'type': 'string', 'enum': enum_values, 'description': clean_description}
        else:
            return {'type': 'string', 'description': clean_description}
...
def organize_fields_by_context(fields):
    ...
    for field in fields:
        ...
        if name in ['items', 'other', 'calculated_inputs', 'device_inputs', 'inputs', 'tires']:
            continue
...
def generate_coordinate_schema(organized_fields):
    """Generate the complete Coordinate schema."""
    schema = {
        'type': 'object',
        'properties': {}
    }
    ...
    # Add position if we have position fields
    ...
    # Add inputs container
    inputs_properties = {}
    
    if organized_fields['other']:
        inputs_properties['other'] = {
            'type': 'object',
    ...
    if organized_fields['tires']:
        tire_properties = create_properties_dict(organized_fields['tires'])
...
def generate_overlay(coordinate_schema):
    """Generate the complete overlay structure."""
    return {
        'overlay': '1.0.0',
        'info': {
            'title': 'Overlay for TrustTrack API',
            'version': '1.0.0'
        },
        'actions': [
            {
    ...
    for i, line in enumerate(lines):
        if '#### Response parameters' in line:
            ...
            for j in range(i, min(i + 10, len(lines))):
                if lines[j].strip().startswith('| Name ') and 'Type' in lines[j] and 'Description' in lines[j]:
                    table_start = j
                    break
            break
...
def main():
    parser = argparse.ArgumentParser(description='Generate OpenAPI overlay from markdown documentation')
    parser.add_argument('markdown_file', help='Path to markdown file containing the field table')
    ...
    overlay = generate_overlay(coordinate_schema)
    ...
    if args.verbose:
        ...
        print(f"  - Root properties: {len(coordinate_schema['properties'])}")
        if 'inputs' in coordinate_schema['properties']:
            inputs_props = coordinate_schema['properties']['inputs']['properties']
            print(f"  - Input sections: {len(inputs_props)}")
            for section, section_data in inputs_props.items():
                if 'properties' in section_data:
                    print(f"    - {section}: {len(section_data['properties'])} properties")
